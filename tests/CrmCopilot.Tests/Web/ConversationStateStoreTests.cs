...
public class ConversationStateStoreTests
{
    ...
    public void GetOrCreate_UnknownSessionId_ReturnsFreshEmptyState()
    {
        ...
        var state = store.GetOrCreate(sessionId);
    ...
    public void Update_UnknownSessionId_CreatesThenAppliesUpdaterAtomically()
    {
        ...
        var result = store.Update(sessionId, s => s with { CurrentCustomerId = "CUS-0001" });

        Assert.Equal("CUS-0001", result.CurrentCustomerId);
        Assert.Equal("CUS-0001", store.GetOrCreate(sessionId).CurrentCustomerId);
    }
    ...
    public void Update_ExistingSession_SeesLatestValue()
    {
        ...
        store.Update(sessionId, s => s with { CurrentCustomerId = "CUS-0001" });
        var result = store.Update(sessionId, s => s with { CurrentCustomerId = "CUS-0002" });

        Assert.Equal("CUS-0002", result.CurrentCustomerId);
        Assert.Equal("CUS-0002", store.GetOrCreate(sessionId).CurrentCustomerId);
    }
    ...
    public void Reset_ThenGetOrCreate_ReturnsFreshState()
    {
        ...
        Assert.Null(store.GetOrCreate(sessionId).CurrentCustomerId);
    }
    ...
    public void Reset_UnknownSessionId_IsNoOp()
    {
        ...
        var exception = Record.Exception(() => store.Reset(Guid.NewGuid().ToString()));
    ...
    public void TwoDifferentSessionIds_NeverShareState()
    {
        ...
        store.Update(sessionA, s => s with { CurrentCustomerId = "CUS-0001" });

        Assert.Equal("CUS-0001", store.GetOrCreate(sessionA).CurrentCustomerId);
        Assert.Null(store.GetOrCreate(sessionB).CurrentCustomerId);
    }
    ...
    public async Task Update_ConcurrentCalls_DoNotLoseWrites()
    {
        ...
        await Task.WhenAll(tags.Select(tag => Task.Run(() =>
            store.Update(sessionId, s => s with { LastInteractionIds = [.. s.LastInteractionIds, tag] }))));

        var final = store.GetOrCreate(sessionId);
